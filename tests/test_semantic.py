...
def test_report_is_partitioned_into_valid_per_specification_runs(tmp_path):
    ...
    for execution in executions:
        ...
        assert str(specifications[0]).rsplit("/", 1)[-1] in expected_specs
...
def test_json_results_use_json_ld_json_literals(tmp_path):
    document = report_to_abox(build_report(10_000, 7), "json-literal-test")
    json_values = [node["mns:resultJson"] for node in document["@graph"] if "mns:resultJson" in node]
    assert json_values
    assert all(value["@type"] == "@json" for value in json_values)

    target = tmp_path / "json-literals.jsonld"
    target.write_text(json.dumps(document), encoding="utf-8")
    assert len(load_abox(target, SCHEMA)) > 0
...
def test_simulation_without_seed_violates_shacl(tmp_path):
    ...
    target.write_text(json.dumps(document), encoding="utf-8")
    outcome = validate_abox(load_abox(target, SCHEMA), SHAPES, load_tbox(TBOX))
