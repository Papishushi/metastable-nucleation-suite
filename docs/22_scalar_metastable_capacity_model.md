...
`distinguishable_states` permanece como entero JSON mientras su valor sea exactamente interoperable en consumidores basados en IEEE 754, hasta \(2^{53}-1\). Los recuentos mayores se conservan sin pérdida mediante `{"encoding": "base16", "value": "0x..."}`. La representación es determinista y reversible con `int(value, 16)` y no modifica el valor de \(K\) empleado por el modelo.
