...
def main() -> int:
    ...
    try:
        if args.experiment == "proportion":
            ...
            result = {"design": "two_proportions", "required_trials_per_group": per_group, "required_trials_total": 2 * per_group}
        elif args.experiment == "correlation":
            if args.rho is None:
                parser.error("correlation planning requires --rho")
            result = {"design": "single_correlation", "required_trials_total": correlation_trials(args.rho, args.alpha, args.power)}
        elif args.experiment == "chsh":
            if args.target_s is None:
                parser.error("CHSH planning requires --target-s")
            per_setting = chsh_trials_per_setting(args.target_s, args.alpha, args.power)
            result = {
                "design": "balanced_chsh",
                "setting_pairs": 4,
                "required_trials_per_setting_pair": per_setting,
                "required_trials_total": 4 * per_setting,
            }
        else:
